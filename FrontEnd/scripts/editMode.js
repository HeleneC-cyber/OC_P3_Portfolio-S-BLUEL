...
export function generateEditMode() {
    ...
    const parentElement = document.querySelector('nav ul li:nth-child(3)')
    const navLoginElement = document.querySelector('a[href="login.html"]')
    const navLogoutElement = document.createElement("a")
    parentElement.replaceChild(navLogoutElement, navLoginElement)
    navLogoutElement.innerText = "logout"
    navLogoutElement.href = "index.html"



    // Création de la barre d'édition
    const editBar = document.createElement("aside")
    ...
    const innerEditBar = `
        ...
        <p>Mode édition</p>
    `
    editBar.innerHTML = innerEditBar

    // Placement de la barre d'édition dans le DOM
    const siblingEditBar = document.querySelector('.container')
    ...
    editTool.classList.add("edit-tool")
    const innerEditTool = `
        ...
        <p>modifier</p>
    `
    editTool.innerHTML = innerEditTool

     // Placement du bouton modifier 
    //  en remplaçant l'élément frère (ici le titre h2) par son futur parent (parentEditTool) qu'on crée (+lui assigne une classe)
    const siblingEditTool = document.querySelector("#portfolio h2")
    const parentEditTool = document.createElement("div")
    parentEditTool.classList.add("title-tool-wrapper")
    const parentSection = document.getElementById("portfolio")
    parentSection.replaceChild(parentEditTool, siblingEditTool)
    // Une fois notre nouvel élément parent placé on lui assigne ses enfants : le titre et le "bouton" modifier
    parentEditTool.appendChild(siblingEditTool)
