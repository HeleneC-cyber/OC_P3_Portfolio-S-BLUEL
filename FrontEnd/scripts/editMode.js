export function generateEditMode() {
    ...
    if(sessionStorage.getItem("token")) {
        const liParent = document.querySelector('nav ul li:nth-child(3)')
        const navElementLogin = document.querySelector('a[href="login.html"]')
        const navElementLogout = document.createElement("a")
        
        liParent.replaceChild(navElementLogout, navElementLogin)
        navElementLogout.innerText = "logout"

        console.log(liParent)
        console.log(navElementLogin)
        console.log(navElementLogout)


        // A FAIRE : Ajouter le bouton d'édition modifier !
        // SVG du bouton :
